...
public abstract class LandICritter extends ICritter {
	...
	/**
	 * <b>interact</b> The LandICritter always adds other LandICritters, but
	 * never MarineICritters.
	 * 
	 * @param other
	...
	public void interact(ICritter other) {
		try {
			...
			if (other instanceof LandICritter) {
				addFriend(other);// add them as a friend since they are a
									// LandICritter
			}
		} catch (NoInterestsException e) {
			e.printStackTrace();
		}
		// LandICritter doesn't add any other types of ICritters as friends,
